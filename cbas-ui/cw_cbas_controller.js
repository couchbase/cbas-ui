...
function cbasController($rootScope, $stateParams, $uibModal, $timeout, cwQueryService, validateCbasService, mnPools, $scope, cwConstantsService, mnPoolDefault, mnAlertsService, mnServersService, $interval, qwJsonCsvService, qwCollectionsService, qwDialogService) {
  ...
  function collapseInputEditor() {
    ...
    qc.filteredDatabaseNames = cwQueryService.filteredDatabaseNames;
    qc.filteredScopes = cwQueryService.filteredScopes;
    ...
    qc.queryContextDatabase = cwConstantsService.defaultQueryContextDatabase;
    ...
    $scope.$watch(() => cwQueryService.loadingBuckets, function () {
      if (!$scope.qc.filteredScopes[$scope.qc.queryContextDatabase] ||
          !$scope.qc.filteredScopes[$scope.qc.queryContextDatabase].includes($scope.qc.queryContextScope)) {
        // reset the query context to Default/Default in the event it becomes invalid
        $scope.qc.queryContextDatabase = cwConstantsService.defaultQueryContextDatabase;
        $scope.qc.queryContextScope = cwConstantsService.defaultQueryContextScope;
      }
    });
    //
  ...
  function getCatalogs() {
    return cwQueryService.catalogs;
  }
  ...
  function getLinksInDataverse(dataverseName) {
    ...
    function databaseNamesToScopeNames(databaseName) {
      return qc.dataverses
...
function createNewCollection(database, dataverse) {
  // Pre-select database and scope if provided
  var defaultDatabaseName = database ? database.DatabaseName : (qc.databases[0]?.DatabaseName || "");
  var defaultScopeName = dataverse ? dataverse.DataverseName : "";
  ...
  createNewCollectionDialogScope.options = {
    ...
    is_new: true,
    databases: qc.databases.map(database => database.DatabaseName),
    dataverses: databaseNamesToScopeNames(defaultDatabaseName),
  ...
  createNewCollectionDialogScope.$watch(
    ...
    (newDatabaseName) => {
      const dataverses = databaseNamesToScopeNames(newDatabaseName);
      createNewCollectionDialogScope.options.dataverses = dataverses;
      if (!createNewCollectionDialogScope.options.scopeName ||
          !dataverses.includes(createNewCollectionDialogScope.options.scopeName)) {
        createNewCollectionDialogScope.options.scopeName = dataverses[0] || "";
      }
    }
  );
  ...
  createNewCollectionDialogScope.model = $uibModal
    .open({
      ...
      if (resp === "ok") {
        const { databaseName, scopeName, collectionName, isAutoGenerated, fields, storageFormat } = createNewCollectionDialogScope.options;
        let queryText;
        if (isAutoGenerated) {
          queryText = `CREATE COLLECTION \`${databaseName}\`.\`${scopeName}\`.\`${collectionName}\` PRIMARY KEY (\`_auto_id\` : uuid) AUTOGENERATED`;
        } else {
          const primaryKeys = fields
              .map(field => `${field.name} : ${field.type}`)
              .join(", ");
          queryText = `CREATE COLLECTION \`${databaseName}\`.\`${scopeName}\`.\`${collectionName}\` PRIMARY KEY (${primaryKeys})`;
        }
        queryText += ` WITH {"storage-format": {"format": "${storageFormat}"}};`;
    ...
    var createNewCatalogDialogScope = $rootScope.$new(true);

    function createNewCatalog() {
      createNewCatalogDialogScope.options = {
        catalog_name: "",
        catalog_source: "",
        catalog_link: "",
        catalog_params: {
      ...
      createNewCatalogDialogScope.deleteAdditionalParam = function(index) {
        createNewCatalogDialogScope.options.additionalParams.splice(index, 1);
      };

      createNewCatalogDialogScope.modal = $uibModal.open({
        template: cwCbasCatalogDialogTemplate,
        scope: createNewCatalogDialogScope
      });

      createNewCatalogDialogScope.submitCatalog = function() {
        cwQueryService.createCatalog(createNewCatalogDialogScope.options)
          .then(function success(resp) {
            createNewCatalogDialogScope.modal.close('ok');
            mnAlertsService.formatAndSetAlerts("Created Iceberg catalog " + createNewCatalogDialogScope.options.catalog_name, 'success', 2000);
            qc.updateBuckets();
          }, function error(resp) {
            createNewCatalogDialogScope.errors = [errorRespToString(resp, "Error creating catalog: ")];
  ...
  // execute a list of statements
    ...
    function generateCreateKafkaCollectionStatement(dataset_options) {
      ...
      statement += ` WITH {"storage-format": {"format": "${dataset_options.storageFormat}"}}`;
    ...
    function createNewDataset(link) {
      ...
      dataset_options.storageFormat = "column";
      // Function to filter dataverses based on the selected database
      ...
      $uibModal.open({
        ...
        .then(function success(resp) {
          ...
          // remote Couchbase: WITH clause must precede ON per grammar (CBDatasetSpecification)
          if (!isExternalLink && dataset_options.link_details.type !== 'kafka')
            queryText += ' WITH {"storage-format": {"format": "' + dataset_options.storageFormat + '"}}';
    ...
    function dropDataset(link, dataset) {
      cwQueryService.showConfirmationDialog("Confirm Drop Collection",
        "Warning, this will drop the collection: ",[dataset.databaseDisplayName + "." + dataset.dataverseDisplayName + "." + dataset.id])
        .then(function yes(resp) {
          if (resp == "ok") {
            // DROP COLLECTION `Default`.`Default`.`coll1`;"
            var queryText = "DROP COLLECTION `" + dataset.DatabaseName + '`.`' + dataset.DataverseName + '`.`' + dataset.id + '`';
    ...
    function dropCatalog(catalog) {
      cwQueryService.showConfirmationDialog("Confirm Drop Catalog",
        "Warning, this will drop the catalog: ",[catalog.CatalogName])
        .then(function yes(resp) {
          if (resp === "ok") {
            var queryText = "DROP CATALOG `" + catalog.CatalogName + "` IF EXISTS;";
            ...
            cwQueryService.executeQueryUtil(queryText, scopesSource, false, false)
              .then(function success() {
                ...
                function error(resp) {
                  console.log("Got drop catalog error: " + JSON.stringify(resp));
                  cwQueryService.showErrorDialog(errorRespToString(resp,"Error dropping catalog: "));
                });
          }
        }, function no() {return Promise.resolve("no")});
    }
    ...
    function dropView(dataverse, view) {
      cwQueryService.showConfirmationDialog("Confirm Drop View",
